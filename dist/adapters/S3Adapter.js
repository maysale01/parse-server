...
S3Adapter.prototype.get = function (config, filename) {
  ...
  return new Promise(function (resolve, reject) {
    _this2.s3.getObject(params, function (err, data) {
      if (err !== null) return reject(err);
      resolve(data.Body);
    });
  });
...
S3Adapter.prototype.location = function (config, req, filename) {
  if (this.directAccess) {
    return 'https://' + this.bucket + '.s3.amazonaws.com' + '/' + this.bucketPrefix + filename;
  }
  return req.protocol + '://' + req.get('host') + path.dirname(req.originalUrl) + '/' + req.config.applicationId + '/' + encodeURIComponent(filename);
};
