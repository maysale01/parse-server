...
function handleFind(req) {
    ...
    return rest.find(req.config, req.auth, '_Installation', req.body.where, options).then(function (response) {
...
function handleGet(req) {
    return rest.find(req.config, req.auth, '_Installation', { objectId: req.params.objectId }).then(function (response) {
...
function handleUpdate(req) {
    return rest.update(req.config, req.auth, '_Installation', req.params.objectId, req.body).then(function (response) {
        return { response: response };
    });
}
...
function handleDelete(req) {
    return rest.del(req.config, req.auth, '_Installation', req.params.objectId).then(function () {
        return { response: {} };
    });
}
