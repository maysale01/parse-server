...
RestQuery.prototype.replaceNotInQuery = function () {
    ...
    return subquery.execute().then(function (response) {
        ...
        // Recurse to repeat
        return _this5.replaceNotInQuery();
    });
};
...
RestQuery.prototype.replaceSelect = function () {
    ...
    if (!selectValue.query || !selectValue.key || _typeof(selectValue.query) !== 'object' || !selectValue.query.className || !selectValue.query.where || Object.keys(selectValue).length !== 2) {
...
RestQuery.prototype.replaceDontSelect = function () {
    ...
    return subquery.execute().then(function (response) {
        ...
        return _this7.replaceDontSelect();
    });
};
...
RestQuery.prototype.runFind = function () {
    ...
    return this.config.database.find(this.className, this.restWhere, this.findOptions).then(function (results) {
        ...
        updateParseFiles(_this8.config, results);

        if (_this8.keys) {
            var keySet = _this8.keys;
        ...
        if (_this8.redirectClassName) {
            ...
            try {
                for (var _iterator8 = results[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
                    ...
                    r.className = _this8.redirectClassName;
                }
            } catch (err) {
        ...
        _this8.response = { results: results };
    });
};
...
RestQuery.prototype.handleInclude = function () {
    ...
    if (pathResponse.then) {
        return pathResponse.then(function (newResponse) {
            _this10.response = newResponse;
            _this10.include = _this10.include.slice(1);
            return _this10.handleInclude();
        });
    }
    return pathResponse;
};
...
function updateParseFiles(config, object) {
    if (object instanceof Array) {
        object.map(function (obj) {
            return updateParseFiles(config, obj);
        });
        return;
    }
    if ((typeof object === 'undefined' ? 'undefined' : _typeof(object)) !== 'object') {
    ...
    for (var key in object) {
        if (object[key] && object[key]['__type'] && object[key]['__type'] == 'File') {
            var filename = object[key]['name'];
            var encoded = encodeURIComponent(filename);
            encoded = encoded.replace('%40', '@');
            if (filename.indexOf('tfss-') === 0) {
                object[key]['url'] = 'http://files.parsetfss.com/' + config.fileKey + '/' + encoded;
            } else {
                object[key]['url'] = config.mount + '/files/' + config.applicationId + '/' + encoded;
            }
        }
    }
}
