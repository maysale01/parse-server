...
function handleParseHeaders(req, res, next) {
  ...
  if (!info.appId || !cache.apps[info.appId]) {
    ...
    if (req.body && req.body._ApplicationId && cache.apps[req.body._ApplicationId] && (!info.masterKey || cache.apps[req.body._ApplicationId]['masterKey'] === info.masterKey)) {
  ...
  return Auth.getAuthForSessionToken(req.config, info.sessionToken).then(function (auth) {
    if (auth) {
      req.auth = auth;
      next();
    }
