...
export default async function handleParseHeaders(req, res, next) {
    try {
        ...
        let app = await cache.getApp(info.appId);
        ...
        if (!info.appId || !app) {
            ...
            if (req.body && req.body._ApplicationId)
                app = await cache.getApp(req.body._ApplicationId);
        ...
        info.app = await cache.getApp(info.appId);
        ...
        let auth = await Auth.getAuthForSessionToken(cache, req.config, info.sessionToken)
        ...
        console.error(`[ParseError]: ${e.message}`, e.stack);
        next(e);
    }
